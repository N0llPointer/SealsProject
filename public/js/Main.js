...
let map = new H.Map(document.getElementById("map"), maptypes.normal.map, {
  zoom: 10,
...
circleArr["Зеленый"] = [
  1,
  new H.map.Circle({ lat: 45.041263083875585, lng: 41.91064660361326 }, 1000, {
    style: { fillColor: "rgba(81, 255, 0, 0.7)" }
  })
];
circleArr["Желтый"] = [
  3,
  new H.map.Circle({ lat: 45.030830722064714, lng: 41.95030038168943 }, 1000, {
    style: { fillColor: "rgba(251, 255, 0, 0.7)" }
  })
];
circleArr["Красный"] = [
  5,
  new H.map.Circle({ lat: 45.04769138288362, lng: 41.97759454062498 }, 1000, {
    style: { fillColor: "rgba(250, 0, 0, 0.7)" }
  })
];
    ...
    if (target.label == undefined) {
      for (var key in eventMarkerDict) {
        if (target.getPosition() == eventMarkerDict[key][1].getPosition()) {
          let eventMarkerWindowTitle = document.querySelector(
            "h5#eventMarkerTitle"
          );
          let eventMarkerWindowDesc = document.querySelector(
            "span#description"
          );
...
function saveBtn() {
  ...
  if (circleArr["Зеленый"][0] == -1) {
    circleArr["Зеленый"][1] = new H.map.Circle(eventCoords, 1000, {
      style: { fillColor: "rgba(81, 255, 0, 0.7)" }
    });
...
getData(function(jsonData) {
  ...
  for (key in jsonText) {
    eventMarkerDict[key] = [
      jsonText[key][0],
      new H.map.Marker(jsonText[key][1])
    ];
